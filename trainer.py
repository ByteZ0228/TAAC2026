...
class DenseParamEMA:
    """EMA over dense parameters only, used for validation/best-checkpoint swap."""

    def __init__(self, params: List[nn.Parameter], decay: float) -> None:
    ...
    @contextmanager
    def apply_shadow(self) -> Iterator[None]:
        backups = [p.detach().clone() for p in self.params]
        try:
            for p, shadow in zip(self.params, self.shadow):
                p.data.copy_(shadow)
            yield
        finally:
            for p, backup in zip(self.params, backups):
                p.data.copy_(backup)
...
class PCVRHyFormerRankingTrainer:
    ...
    def __init__(
        ...
        if hasattr(model, 'get_dense_params'):
            self._ema_params = model.get_dense_params()
        else:
            self._ema_params = [p for p in model.parameters() if p.requires_grad]
        self.ema: Optional[DenseParamEMA] = None
        if self.use_ema:
            self.ema = DenseParamEMA(self._ema_params, decay=self.ema_decay)
            logging.info(
                f"EMA enabled for validation/best checkpoint on {len(self._ema_params)} "
                f"dense params, decay={self.ema_decay}"
            )
    ...
    def train(self) -> None:
        ...
        for epoch in range(1, self.num_epochs + 1):
            ...
            # Save one checkpoint every epoch regardless of best-model status.
    ...
    def _make_model_input(self, device_batch: Dict[str, Any]) -> ModelInput:
        ...
        seq_time_periods: Dict[str, torch.Tensor] = {}
        for domain in seq_domains:
            ...
            seq_time_periods[domain] = device_batch.get(
                f'{domain}_time_period',
                torch.zeros(B, L, 4, dtype=torch.float32, device=self.device))
        return ModelInput(
            user_int_feats=device_batch['user_int_feats'],
            item_int_feats=device_batch['item_int_feats'],
            user_dense_feats=device_batch['user_dense_feats'],
            item_dense_feats=device_batch['item_dense_feats'],
            request_time_int_feats=device_batch['request_time_int_feats'],
            request_time_sincos_feats=device_batch['request_time_sincos_feats'],
            seq_data=seq_data,
            seq_lens=seq_lens,
            seq_time_buckets=seq_time_buckets,
            seq_time_periods=seq_time_periods,
        )
