...
BEIJING_TIME_OFFSET_SECONDS = 8 * 3600
BEIJING_TIMEZONE = timezone(timedelta(hours=8))
NUM_REQUEST_TIME_PARTS = 6
...
class PCVRParquetDataset(IterableDataset):
    ...
    def _convert_batch(self, batch: "pa.RecordBatch") -> Dict[str, Any]:
        ...
        local_datetimes = [
            datetime.fromtimestamp(int(ts), tz=timezone.utc).astimezone(BEIJING_TIMEZONE)
            for ts in timestamps
        ]
        hour_of_day = np.array([dt.hour for dt in local_datetimes], dtype=np.int64)
        day_of_week = np.array([dt.weekday() for dt in local_datetimes], dtype=np.int64)
        day_of_month = np.array([dt.day for dt in local_datetimes], dtype=np.int64)
        part_day = np.clip(hour_of_day // 4, 0, NUM_REQUEST_TIME_PARTS - 1).astype(np.int64)
        request_time_int_feats = np.stack([
            hour_of_day,
            day_of_week,
            day_of_month,
            part_day,
        ...
        sec_of_day = np.array(
            [dt.hour * 3600 + dt.minute * 60 + dt.second for dt in local_datetimes],
            dtype=np.float32,
        )
        day_of_week_float = day_of_week.astype(np.float32)
        day_of_month_pos = np.array(
            [
                (dt.day - 1) / max(calendar.monthrange(dt.year, dt.month)[1], 1)
                for dt in local_datetimes
            ],
            dtype=np.float32,
        )
        day_of_year_pos = np.array(
            [
                (dt.timetuple().tm_yday - 1) / (366.0 if calendar.isleap(dt.year) else 365.0)
                for dt in local_datetimes
            ],
            dtype=np.float32,
        )
        request_time_sincos_feats = np.stack([
            np.sin(2.0 * np.pi * sec_of_day / 86400.0),
            np.cos(2.0 * np.pi * sec_of_day / 86400.0),
            np.sin(2.0 * np.pi * day_of_week_float / 7.0),
            np.cos(2.0 * np.pi * day_of_week_float / 7.0),
            np.sin(2.0 * np.pi * day_of_month_pos),
            np.cos(2.0 * np.pi * day_of_month_pos),
            np.sin(2.0 * np.pi * day_of_year_pos),
            np.cos(2.0 * np.pi * day_of_year_pos),
        ...
        result = {
            ...
            'request_time_int_feats': torch.from_numpy(request_time_int_feats.copy()),
            'request_time_sincos_feats': torch.from_numpy(request_time_sincos_feats.copy()),
            'user_id': user_ids,
        ...
        for domain in self.seq_domains:
            ...
            time_bucket[:] = 0
            time_period = np.zeros((B, max_len, 4), dtype=np.float32)
            if ts_ci is not None:
                ...
                if valid_ts.any():
                    local_ts = ts_padded.astype(np.float64) + BEIJING_TIME_OFFSET_SECONDS
                    hour = (np.remainder(local_ts, 86400.0) / 3600.0)
                    hour_angle = hour * (2.0 * np.pi / 24.0)
                    day_idx = np.floor_divide(local_ts, 86400.0)
                    weekday = np.remainder(day_idx + 4.0, 7.0)
                    weekday_angle = weekday * (2.0 * np.pi / 7.0)

                    time_period[..., 0] = np.sin(hour_angle)
                    time_period[..., 1] = np.cos(hour_angle)
                    time_period[..., 2] = np.sin(weekday_angle)
                    time_period[..., 3] = np.cos(weekday_angle)
                    time_period[~valid_ts] = 0.0
            ...
            result[f'{domain}_time_period'] = torch.from_numpy(time_period.copy())
