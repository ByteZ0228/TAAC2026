...
class ModelInput(NamedTuple):
    ...
    request_time_int_feats: torch.Tensor  # [B, 4]
    request_time_sincos_feats: torch.Tensor  # [B, 8]
    seq_data: dict        # {domain: tensor [B, S, L]}
    seq_lens: dict        # {domain: tensor [B]}
    seq_time_buckets: dict  # {domain: tensor [B, L]}
    seq_time_periods: dict  # {domain: tensor [B, L, 4]}
...
class MultiSeqQueryGenerator(nn.Module):
    ...
    def __init__(
        ...
        self.item_context_norm = nn.LayerNorm(d_model)
        ...
        self.target_pool_mlps = nn.ModuleList([
            nn.Sequential(
        ...
        for mlp in self.target_pool_mlps:
    ...
    def forward(
        ...
        Args:
            ...
            item_tokens: (B, Mi, D), item-side NS tokens.
        ...
        item_ctx = self.item_context_norm(item_tokens.mean(dim=1))  # (B, D)
        ...
        for i in range(self.num_sequences):
            ...
            item_ctx_expanded = item_ctx.unsqueeze(1).expand(-1, seq_tokens.shape[1], -1)

            # DIN-style target-aware weighted pooling over history.
            score_input = torch.cat([
                seq_tokens,
                item_ctx_expanded,
                seq_tokens * item_ctx_expanded,
                seq_tokens - item_ctx_expanded,
            ], dim=-1)
            scores = self.target_pool_mlps[i](score_input).squeeze(-1)  # (B, L_i)
            scores = scores.masked_fill(~valid_mask, float('-inf'))
            attn = torch.softmax(scores, dim=1)
            attn = torch.nan_to_num(attn, nan=0.0).unsqueeze(-1)  # (B, L_i, 1)
            seq_pooled = (seq_tokens * attn * valid_mask_expanded).sum(dim=1)  # (B, D)
...
class MultiSeqHyFormerBlock(nn.Module):
    ...
    def __init__(
        ...
        self.num_queries = num_queries
        self.num_ns = num_ns
        self.domain_gate_ns_norm = nn.LayerNorm(d_model)
        self.domain_gate_ctx_norm = nn.LayerNorm(d_model)
        self.domain_gate_score = nn.Sequential(
            nn.LayerNorm(3 * d_model),
            nn.Linear(3 * d_model, d_model),
            nn.SiLU(),
            nn.Linear(d_model, 1),
        )
        self.domain_context_proj = nn.Sequential(
            nn.LayerNorm(d_model),
            nn.Linear(d_model, d_model),
            nn.SiLU(),
            nn.Linear(d_model, d_model),
        )
        nn.init.zeros_(self.domain_context_proj[-1].weight)
        nn.init.zeros_(self.domain_context_proj[-1].bias)
    ...
    def _apply_domain_gate(
        ...
        """NS-conditioned domain gate before sequence/query updates."""
        ns_global = self.domain_gate_ns_norm(ns_tokens.mean(dim=1))  # (B, D)

        domain_contexts = []
        domain_scores = []
        for seq_tokens, seq_mask in zip(seq_tokens_list, seq_padding_masks):
            valid_mask = (~seq_mask).unsqueeze(-1).float()  # (B, L, 1)
            seq_sum = (seq_tokens * valid_mask).sum(dim=1)
            seq_count = valid_mask.sum(dim=1).clamp(min=1.0)
            domain_ctx = self.domain_gate_ctx_norm(seq_sum / seq_count)  # (B, D)
            score_input = torch.cat([
                ns_global,
                domain_ctx,
                ns_global * domain_ctx,
            ], dim=-1)
            domain_score = self.domain_gate_score(score_input)  # (B, 1)
            domain_contexts.append(domain_ctx)
            domain_scores.append(domain_score)

        domain_scores_tensor = torch.cat(domain_scores, dim=1)  # (B, S)
        domain_weights = torch.softmax(domain_scores_tensor, dim=1)  # (B, S)
        stacked_contexts = torch.stack(domain_contexts, dim=1)  # (B, S, D)
        domain_context = (stacked_contexts * domain_weights.unsqueeze(-1)).sum(dim=1)  # (B, D)
        ns_delta = self.domain_context_proj(domain_context).unsqueeze(1)  # (B, 1, D)
        return ns_tokens + ns_delta
    ...
    def forward(
        ...
        # 0. NS-conditioned dynamic domain gate
        ns_tokens = self._apply_domain_gate(ns_tokens, seq_tokens_list, seq_padding_masks)
...
class GroupNSTokenizer(nn.Module):
    ...
    def __init__(self, feature_specs: List[Tuple[int, int, int]],
                 ...
                 paired_dense_overrides: Optional[dict] = None,
                 ignored_feature_indices: Optional[set] = None) -> None:
        super().__init__()
        self.feature_specs = feature_specs
        self.groups = groups
        self.emb_dim = emb_dim
        self.emb_skip_threshold = emb_skip_threshold
        self.paired_dense_overrides = paired_dense_overrides or {}
        self.ignored_feature_indices = ignored_feature_indices or set()
    ...
    def forward(self, int_feats: torch.Tensor, dense_feats: Optional[torch.Tensor] = None) -> torch.Tensor:
        ...
        for group, proj in zip(self.groups, self.group_projs):
            ...
            for fid_idx in group:
                ...
                if fid_idx in self.ignored_feature_indices:
                    fid_embs.append(int_feats.new_zeros(int_feats.shape[0], self.emb_dim, dtype=torch.float32))
                    continue
                ...
                if emb_real_idx == -1:
                    ...
                    if length == 1:
                        ...
                        mask = (vals != 0).float()
                        if fid_idx in self.paired_dense_overrides and dense_feats is not None:
                            dense_offset, dense_length = self.paired_dense_overrides[fid_idx]
                            dense_vals = dense_feats[:, dense_offset:dense_offset + dense_length]
                            weights = torch.log1p(torch.clamp_min(dense_vals, 0.0)) * mask
                            weight_sum = weights.sum(dim=1, keepdim=True).clamp(min=1e-6)
                            fid_emb = (emb_all * weights.unsqueeze(-1)).sum(dim=1) / weight_sum
                        else:
                            mask = mask.unsqueeze(-1)  # (B, length, 1)
...
class RankMixerNSTokenizer(nn.Module):
    ...
    def __init__(
        ...
        emb_skip_threshold: int = 0,
        paired_dense_overrides: Optional[dict] = None,
        ignored_feature_indices: Optional[set] = None,
    ) -> None:
    ...
    def forward(self, int_feats: torch.Tensor, dense_feats: Optional[torch.Tensor] = None) -> torch.Tensor:
        ...
        for group in self.groups:
            for fid_idx in group:
                vs, offset, length = self.feature_specs[fid_idx]
                if fid_idx in self.ignored_feature_indices:
                    all_embs.append(int_feats.new_zeros(int_feats.shape[0], self.emb_dim, dtype=torch.float32))
                    continue
                emb_real_idx = self._emb_index[fid_idx]
                if emb_real_idx == -1:
                    fid_emb = int_feats.new_zeros(int_feats.shape[0], self.emb_dim)
...
class QuantileTrendConvEncoder(nn.Module):
    """Encode user_dense fids 89/90/91 as a 3-channel trend sequence."""

    def __init__(self, d_model: int, hidden_dim: Optional[int] = None) -> None:
        super().__init__()
        hidden_dim = hidden_dim or d_model
        self.hidden_dim = hidden_dim
        self.conv1 = nn.Conv1d(3, hidden_dim, kernel_size=3, padding=1)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """Args: x of shape (B, 3, L). Returns (B, hidden_dim)."""
        x = self.conv1(x)
        x = F.silu(x)
        x = self.conv2(x)
...
class RequestTimeEncoder(nn.Module):
    """Encode 4 request-time categorical features into one dense context vector."""
    ...
    def __init__(self, d_model: int, emb_dim: int) -> None:
        ...
        self.day_of_month_emb = nn.Embedding(32, emb_dim)
        self.part_day_emb = nn.Embedding(6, emb_dim)
        self.proj = nn.Sequential(
            nn.Linear(4 * emb_dim, d_model),
            nn.LayerNorm(d_model),
        )

    def forward(self, request_time_int_feats: torch.Tensor) -> torch.Tensor:
        hour = self.hour_emb(request_time_int_feats[:, 0].long())
        weekday = self.weekday_emb(request_time_int_feats[:, 1].long())
        day_of_month = self.day_of_month_emb(request_time_int_feats[:, 2].long())
        part_day = self.part_day_emb(request_time_int_feats[:, 3].long())
        cat = torch.cat([hour, weekday, day_of_month, part_day], dim=-1)
        return F.silu(self.proj(cat))


class RequestTimeSinCosEncoder(nn.Module):
    """Encode 8-d request-time cyclical sin/cos features into one dense context vector."""
    ...
    def forward(self, request_time_sincos_feats: torch.Tensor) -> torch.Tensor:
        return F.silu(self.proj(request_time_sincos_feats.float()))


class DenseGroupFFN(nn.Module):
    """A lightweight residual FFN applied after each dense-group encoder."""
...
class PCVRHyFormer(nn.Module):
    ...
    def __init__(
        ...
        user_int_feature_specs_with_fid: Optional[List[Tuple[int, int, int, int]]] = None,
        ...
        use_time_period_feats: bool = True,
        rank_mixer_mode: str = 'full',
        use_rope: bool = False,
        ...
        self.use_time_period_feats = use_time_period_feats
        ...
        self.user_int_feature_specs_with_fid = user_int_feature_specs_with_fid or []
        self.user_dense_single_fids = [61, 87]
        self.user_quantile_trend_fids = [89, 90, 91]
        self.user_drop_int_fids = [89, 90, 91]
        self.user_paired_varlen_fids = [62, 63, 64, 65, 66]
        self.user_dense_single_specs = [
            spec for spec in self.user_dense_feature_specs
            if spec[0] in self.user_dense_single_fids
        ]
        self.user_dense_single_fid_to_spec = {
            fid: (offset, length) for fid, offset, length in self.user_dense_single_specs
        }
        self.user_quantile_trend_specs = [
            spec for spec in self.user_dense_feature_specs
            if spec[0] in self.user_quantile_trend_fids
        ]
        self.user_quantile_trend_fid_to_spec = {
            fid: (offset, length) for fid, offset, length in self.user_quantile_trend_specs
        }
        self.user_paired_varlen_specs = [
            spec for spec in self.user_dense_feature_specs
            if spec[0] in self.user_paired_varlen_fids
        ]
        self.user_paired_varlen_fid_to_spec = {
            fid: (offset, length) for fid, offset, length in self.user_paired_varlen_specs
        }
        self.user_int_fid_to_feature_idx = {
            fid: i for i, (fid, _, _, _) in enumerate(self.user_int_feature_specs_with_fid)
        }
        self.user_ignored_int_feature_indices = {
            self.user_int_fid_to_feature_idx[fid]
            for fid in self.user_drop_int_fids
            if fid in self.user_int_fid_to_feature_idx
        }
        self.user_paired_dense_overrides = {
            self.user_int_fid_to_feature_idx[fid]: self.user_paired_varlen_fid_to_spec[fid]
            for fid in self.user_paired_varlen_fids
            if fid in self.user_int_fid_to_feature_idx and fid in self.user_paired_varlen_fid_to_spec
        }
        ...
        if ns_tokenizer_type == 'group':
            ...
            self.user_ns_tokenizer = GroupNSTokenizer(
                ...
                paired_dense_overrides=self.user_paired_dense_overrides,
                ignored_feature_indices=self.user_ignored_int_feature_indices,
            )
        ...
        if self.has_user_dense:
            self.has_dense_61 = 61 in self.user_dense_single_fid_to_spec
            self.has_dense_87 = 87 in self.user_dense_single_fid_to_spec
            self.has_quantile_trend_dense = len(self.user_quantile_trend_specs) == 3
            self.has_paired_varlen_dense = len(self.user_paired_varlen_specs) > 0
            self.user_dense_scalar_branch_dim = 0
            if self.has_dense_61:
                _, dense_61_len = self.user_dense_single_fid_to_spec[61]
                self.user_dense_61_proj = nn.Sequential(
                    nn.Linear(dense_61_len, d_model),
                    nn.LayerNorm(d_model),
                )
                self.user_dense_61_ffn = DenseGroupFFN(d_model)
                self.user_dense_scalar_branch_dim += d_model
            else:
                self.user_dense_61_proj = None
                self.user_dense_61_ffn = None
            if self.has_dense_87:
                _, dense_87_len = self.user_dense_single_fid_to_spec[87]
                self.user_dense_87_proj = nn.Sequential(
                    nn.Linear(dense_87_len, d_model),
                    nn.LayerNorm(d_model),
                )
                self.user_dense_87_ffn = DenseGroupFFN(d_model)
                self.user_dense_scalar_branch_dim += d_model
            else:
                self.user_dense_87_proj = None
                self.user_dense_87_ffn = None
            if self.has_quantile_trend_dense:
                trend_lengths = {
                    self.user_quantile_trend_fid_to_spec[fid][1]
                    for fid in self.user_quantile_trend_fids
                }
                if len(trend_lengths) != 1:
                    raise ValueError(
                        f"user_dense fids 89/90/91 must share the same length, got {trend_lengths}"
                    )
                self.user_quantile_trend_len = next(iter(trend_lengths))
                self.user_quantile_trend_encoder = QuantileTrendConvEncoder(d_model)
                self.user_quantile_trend_ffn = DenseGroupFFN(d_model)
            else:
                self.user_quantile_trend_len = 0
                self.user_quantile_trend_encoder = None
                self.user_quantile_trend_ffn = None
            if self.has_paired_varlen_dense:
                self.user_paired_varlen_dense_fids_present = [
                    fid for fid in self.user_paired_varlen_fids
                    if fid in self.user_paired_varlen_fid_to_spec
                ]
                self.user_paired_varlen_total_dim = sum(
                    self.user_paired_varlen_fid_to_spec[fid][1]
                    for fid in self.user_paired_varlen_dense_fids_present
                )
                self.user_paired_varlen_stats_dim = 2 * len(self.user_paired_varlen_dense_fids_present)
                self.user_paired_varlen_proj = nn.Sequential(
                    nn.Linear(
                        self.user_paired_varlen_total_dim + self.user_paired_varlen_stats_dim,
                        d_model,
                    ),
                    nn.LayerNorm(d_model),
                )
                self.user_paired_varlen_ffn = DenseGroupFFN(d_model)
            else:
                self.user_paired_varlen_dense_fids_present = []
                self.user_paired_varlen_total_dim = 0
                self.user_paired_varlen_stats_dim = 0
                self.user_paired_varlen_proj = None
                self.user_paired_varlen_ffn = None
            self.request_time_encoder = RequestTimeEncoder(d_model=d_model, emb_dim=emb_dim)
            self.request_time_ffn = DenseGroupFFN(d_model)
            self.request_time_sincos_encoder = RequestTimeSinCosEncoder(d_model=d_model)
            self.request_time_sincos_ffn = DenseGroupFFN(d_model)
            dense_branch_dim = (
                self.user_dense_scalar_branch_dim
                + (d_model if self.user_quantile_trend_encoder is not None else 0)
                + (d_model if self.user_paired_varlen_proj is not None else 0)
                + d_model
                + d_model
            )
            if dense_branch_dim > 0:
                self.user_dense_fuse = nn.Sequential(
                    nn.Linear(dense_branch_dim, d_model),
                    nn.LayerNorm(d_model),
                )
            else:
                self.user_dense_fuse = None
            self.num_user_dense_tokens = int(dense_branch_dim > 0)
        else:
        ...
        if use_time_period_feats:
            self.time_period_proj = nn.Linear(4, d_model, bias=False)
    ...
    def _init_params(self) -> None:
        ...
        if getattr(self, 'request_time_encoder', None) is not None:
            for emb in [
                self.request_time_encoder.hour_emb,
                self.request_time_encoder.weekday_emb,
                self.request_time_encoder.day_of_month_emb,
                self.request_time_encoder.part_day_emb,
            ]:
                nn.init.xavier_normal_(emb.weight.data)
        ...
        if self.use_time_period_feats:
            nn.init.xavier_normal_(self.time_period_proj.weight.data)

    def _build_user_paired_varlen_dense_token(
        self,
        user_dense_feats: torch.Tensor,
    ) -> Optional[torch.Tensor]:
        """Encode dense fids 62-66 as one grouped branch."""
        if self.user_paired_varlen_proj is None:
            return None

        unit_parts = []
        stat_parts = []
        for fid in self.user_paired_varlen_dense_fids_present:
            offset, length = self.user_paired_varlen_fid_to_spec[fid]
            dense_vals = user_dense_feats[:, offset:offset + length]
            dense_vals = torch.log1p(torch.clamp_min(dense_vals, 0.0))
            unit_parts.append(F.normalize(dense_vals, p=2, dim=-1, eps=1e-6))
            stat_parts.append(dense_vals.mean(dim=-1, keepdim=True))
            stat_parts.append(torch.linalg.vector_norm(dense_vals, ord=2, dim=-1, keepdim=True))

        if not unit_parts:
            return None

        group_input = torch.cat(unit_parts + stat_parts, dim=-1)
        group_tok = F.silu(self.user_paired_varlen_proj(group_input))
        return self.user_paired_varlen_ffn(group_tok)

    def _build_user_dense_tokens(
        self,
        user_dense_feats: torch.Tensor,
        user_int_feats: torch.Tensor,
        request_time_int_feats: torch.Tensor,
        request_time_sincos_feats: torch.Tensor,
    ) -> torch.Tensor:
        """Build one shared user-dense token from grouped dense/time branches."""
        parts = []
        if self.user_dense_61_proj is not None:
            offset, length = self.user_dense_single_fid_to_spec[61]
            dense_61 = user_dense_feats[:, offset:offset + length]
            dense_61 = F.normalize(dense_61, p=2, dim=-1, eps=1e-6)
            dense_61_tok = F.silu(self.user_dense_61_proj(dense_61))
            parts.append(self.user_dense_61_ffn(dense_61_tok))
        if self.user_dense_87_proj is not None:
            offset, length = self.user_dense_single_fid_to_spec[87]
            dense_87 = user_dense_feats[:, offset:offset + length]
            dense_87 = F.normalize(dense_87, p=2, dim=-1, eps=1e-6)
            dense_87_tok = F.silu(self.user_dense_87_proj(dense_87))
            parts.append(self.user_dense_87_ffn(dense_87_tok))

        if self.user_quantile_trend_encoder is not None:
            trend_channels = []
            for fid in self.user_quantile_trend_fids:
                offset, length = self.user_quantile_trend_fid_to_spec[fid]
                trend_channels.append(user_dense_feats[:, offset:offset + length].unsqueeze(1))
            trend_seq = torch.cat(trend_channels, dim=1)  # (B, 3, L)
            trend_tok = self.user_quantile_trend_encoder(trend_seq)
            parts.append(self.user_quantile_trend_ffn(trend_tok))

        paired_varlen_tok = self._build_user_paired_varlen_dense_token(user_dense_feats)
        if paired_varlen_tok is not None:
            parts.append(paired_varlen_tok)

        request_time_tok = self.request_time_encoder(request_time_int_feats)
        parts.append(self.request_time_ffn(request_time_tok))

        request_time_sincos_tok = self.request_time_sincos_encoder(request_time_sincos_feats)
        parts.append(self.request_time_sincos_ffn(request_time_sincos_tok))
        ...
        return F.silu(self.user_dense_fuse(fused)).unsqueeze(1)
    ...
    def _embed_seq_domain(
        ...
        time_period_feats: torch.Tensor,
    ) -> torch.Tensor:
        ...
        if self.use_time_period_feats:
            token_emb = token_emb + self.time_period_proj(time_period_feats.float())
    ...
    def forward(self, inputs: ModelInput) -> torch.Tensor:
        ...
        user_ns = self.user_ns_tokenizer(inputs.user_int_feats, inputs.user_dense_feats)   # (B, num_user_groups, D)
        item_ns = self.item_ns_tokenizer(inputs.item_int_feats)   # (B, num_item_groups, D)

        ns_parts = [user_ns]
        item_parts = [item_ns]
        if self.has_user_dense:
            user_dense_toks = self._build_user_dense_tokens(
                inputs.user_dense_feats, inputs.user_int_feats,
                inputs.request_time_int_feats, inputs.request_time_sincos_feats)  # (B, Nu_dense, D)
            if user_dense_toks.shape[1] > 0:
        ...
        item_tokens = torch.cat(item_parts, dim=1)  # (B, num_item_tokens, D)
        ...
        for domain in self.seq_domains:
            tokens = self._embed_seq_domain(
                ...
                inputs.seq_time_periods[domain])
            seq_tokens_list.append(tokens)
            mask = self._make_padding_mask(inputs.seq_lens[domain], inputs.seq_data[domain].shape[2])
            seq_masks_list.append(mask)

        # 3. Generate independent Q tokens per sequence via MultiSeqQueryGenerator
        q_tokens_list = self.query_generator(ns_tokens, item_tokens, seq_tokens_list, seq_masks_list)
    ...
    def predict(self, inputs: ModelInput) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        user_ns = self.user_ns_tokenizer(inputs.user_int_feats, inputs.user_dense_feats)
        ...
        if self.has_user_dense:
            user_dense_toks = self._build_user_dense_tokens(
                inputs.user_dense_feats, inputs.user_int_feats,
                inputs.request_time_int_feats, inputs.request_time_sincos_feats)
            if user_dense_toks.shape[1] > 0:
                ns_parts.append(user_dense_toks)
        ns_parts.append(item_ns)
        if self.has_item_dense:
